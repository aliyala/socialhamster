...
export class CommentDataSource extends DataSource {
  ...
  constructor(comments: Comment[]) {
    ...
    this.commentLoader = new DataLoader<string, Comment>(this.batchLoadComments.bind(this));
  }
  ...
  async batchLoadComments(commentIds: string[]): Promise<Comment[]> {
    const filteredComments = this.comments.filter((comment) => commentIds.includes(comment.id));
  ...
  async getComments(commentIds: string[]): Promise<Comment[]> {
   ...
   const filteredComments = comments.filter((comment): comment is Comment => comment !== undefined);
